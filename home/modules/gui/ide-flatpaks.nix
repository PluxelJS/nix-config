...
${renderShellArrayItems jetbrainsOverrideArgs}
  ...
  home.activation.prepareFlatpakIdeCodexHomes = lib.hm.dag.entryAfter [ "manageFlatpakIdeOverrides" ] ''
    host_codex_config='${homeDir}/.codex/config.toml'
    ...
    done < <(find "${homeDir}/.var/app" -maxdepth 1 -mindepth 1 -type d \( -name 'com.jetbrains.*' -o -name '${codeStudioAppId}' \) 2>/dev/null | sort)
  '';
