...
Home Manager also links the host-owned fontconfig, fonts, GTK/KDE settings,
icons, themes, sounds, and Fcitx resources into their normal locations below
the private home. Their targets are exposed by the global Flatpak override as
read-only paths. Development-tool state remains private; the bridge contains
only the shared desktop integration allowlist.

## Version Pin

Code Studio deliberately pins VS Code `1.116.0`
(`560a9dba96f961efea7b1612916f89e5d5d4d679`) with the Freedesktop `24.08`
SDK as a tested rendering compatibility baseline. Newer tested VS Code
`1.124.x` / Electron 42 builds rendered editor text visibly blurrier on this
Wayland setup, including with equivalent fontconfig visibility and settings.

This compatibility baseline is exempt from routine dependency refreshes. Only
change it after explicitly comparing font clarity and terminal integration on
the target Wayland desktop.
